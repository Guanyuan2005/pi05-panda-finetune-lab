        ...
        try:
            for step_index in range(args.max_steps):
                ...
                for action in action_seq:
                    start_q = data.qpos[ids.arm_qpos].copy()
                    start_g = env.normalized_gripper_state(data, ids)
                    target_q = np.asarray(action[:7], dtype=float)
                    target_g = float(action[7])
                    for interp_step in range(physics_steps):
                        alpha = (interp_step + 1) / physics_steps
                        q_target = start_q + alpha * (target_q - start_q)
                        g_target = start_g + alpha * (target_g - start_g)
                        data.ctrl[ids.arm_actuator] = q_target
                        data.ctrl[ids.gripper_actuator] = env.actuator_gripper(model, ids, g_target)
